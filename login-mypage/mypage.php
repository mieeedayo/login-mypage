...
if (empty($_SESSION['id'])) {
  try {
    ...
    die("<p>申し訳ございません。現在サーバーが込み合っており一時的にアクセスができません。<br>しばらくしてから再度ログインしてください。</p><a href='http://localhost/login_mypage/login.php'>ログイン画面へ</a>");
  }

  //プリペアでSQL分の方を作るDBとPOSTデータを照合。select文とwhere区を使う
  ...
  //データを取得ができずにselectがなければ、リダイレクト（エラー画面へ）
  if (empty($_SESSION['id'])) {
